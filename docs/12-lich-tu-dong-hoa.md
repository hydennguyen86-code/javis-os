...
Trang **Lịch** là nơi bạn ghi lại và quản lý các việc chạy tự động của Javis: lịch theo giờ (cron), việc kích hoạt theo sự kiện (trigger), và routine (agent chạy theo lịch). Đây cũng là chỗ bật/tắt nhanh **Vòng lặp tự cải thiện** - phần lõi tự chạy nền của Javis.

## Tính năng này là gì

Javis có thể làm việc mà không cần bạn ngồi canh: mỗi sáng tổng hợp số liệu, định kỳ làm dày bộ não, hay chạy một routine trên nền tảng đám mây của Claude. Trang Lịch gom tất cả các "việc tự chạy" đó vào một danh sách để bạn nhìn thấy cái nào đang bật, cái nào đang tắt, chạy theo lịch gì.
...
Một điểm cần hiểu ngay: phần lớn mục bạn thêm ở đây là **bản ghi nhớ** (registry). Javis lưu lại "tôi có một lịch tên X, chạy lúc Y" để bạn không quên, nhưng bản thân dashboard không tự bấm giờ chạy các mục cron/trigger đó. Việc thật sự chạy đúng lịch trên nền là **Vòng lặp tự cải thiện** (chạy trong máy Javis) và các **routine đám mây** (chạy trên Claude cloud, kéo về qua nút Đồng bộ). Phần dưới nói rõ từng loại.

## Mở ở đâu trong Javis

1. Mở dashboard Javis (mặc định ở địa chỉ máy bạn, cổng `7777`).
...
> Lưu ý về ô "Lịch / mô tả": ô này nhận **chữ tự do**, không phải cú pháp cron kỹ thuật. Bạn cứ viết bằng tiếng Việt cho dễ đọc. Đây là bản ghi nhớ để bạn và Javis cùng biết lịch dự kiến, không phải nơi hệ thống tự đặt đồng hồ đếm giờ.
...
1. Trên thẻ, bấm **Xoá**.
2. Xác nhận khi Javis hỏi "Xoá ...?".

Mục 🔁 không xoá được, chỉ tắt được. Nếu cố xoá, Javis báo "Loop nội bộ chỉ tắt được, không xoá".
...
2. Javis nhờ Claude liệt kê các routine đang chạy trên cloud (chỉ liệt kê, không tạo/sửa/xoá gì).
3. Kết quả:
   - Tìm thấy: các mục hiện ra với biểu tượng ☁ ở đầu tên, kèm ghi chú "☁ cloud" và thời điểm chạy kế tiếp nếu có. Javis báo "Đã đồng bộ N routine từ cloud."
...
- Trang **Tự cải thiện**: nơi cấu hình đầy đủ - chọn loại nhiệm vụ (Kinh doanh, Bộ não/Wiki, Cải thiện Javis, hoặc Tự định nghĩa), chế độ (Đề xuất ghi nháp / Tự làm + kiểm chứng), và **Chu kỳ (phút)**.
...
- Javis kiểm tra mỗi 30 giây một lần xem đã tới lượt chưa.
- Nếu vòng đang bật, và thời gian kể từ lần chạy gần nhất đã bằng hoặc vượt chu kỳ bạn đặt (ví dụ 60 phút), Javis chạy một vòng.
- Chu kỳ tối thiểu là 5 phút. Đặt nhỏ hơn cũng bị ép về 5.
- Nếu đang có một vòng chạy dở, Javis bỏ qua để không chạy chồng.
...
- Muốn Javis tự làm việc đều đặn ngay trong máy bạn, hãy dùng **Vòng lặp tự cải thiện** thay vì các mục cron ghi tay: đó mới là phần thực sự tự chạy theo lịch. Cấu hình ở trang [Tự cải thiện](08-tu-cai-thien.md).
- Nếu muốn Javis nhắc bạn kết quả qua điện thoại, kết hợp với kênh [Telegram](11-telegram.md).
...
Nghĩa là Claude CLI nền chưa thấy công cụ lịch (MCP), hoặc trả về không đúng định dạng. Xem thêm phần Claude trả về trong hộp báo lỗi. Nếu chưa cài Claude CLI, Javis báo "Claude CLI chưa cài" - cài và đăng nhập trước ở trang [Models & engine](10-models-va-engine.md).

**Đã thêm một lịch cron/trigger nhưng nó không tự chạy.**
Các mục bạn ghi tay chủ yếu là bản ghi nhớ, dashboard không tự đặt đồng hồ chạy chúng. Việc chạy thật theo lịch là **Vòng lặp tự cải thiện** (trong máy Javis) và **routine đám mây** (kéo về qua Đồng bộ). Nếu cần Javis tự làm định kỳ, dùng trang [Tự cải thiện](08-tu-cai-thien.md).
...
Thường do máy chủ Javis chưa chạy hoặc phiên đăng nhập hết hạn. Tải lại trang, đăng nhập lại, và nếu cần khởi động lại server. Xem [Khắc phục sự cố & FAQ](17-khac-phuc-su-co.md).
