...
qua nguyên vẹn, và (ép bằng Event, không sleep) 2 luồng đua khi cây ĐÃ đổi chỉ ra đúng MỘT
lượt copy thật - chứng minh lock riêng của mirror thật sự có tác dụng, không chỉ là lock
cho có.
"""
import os
import shutil
...
# ---- lock CHỈ đáng tồn tại nếu chặn được copy TRÙNG khi cây ĐÃ đổi ----
# Hai check 4-luồng ở trên KHÔNG chứng minh lock có tác dụng: cả hai được giải thích đầy đủ
# bởi bug copy-nông đã sửa (xem task-1-report.md). Test này ép đúng khe hở nguy hiểm của
# check-rồi-khoá: luồng B đọc chữ ký (thấy cây đã đổi, cache còn cũ) RỒI mới xin khoá - nếu
# đúng lúc đó luồng A đã copy xong + cập nhật cache + NHẢ khoá, B xin khoá THÀNH CÔNG (khoá
# đang rảnh) và nếu B không kiểm lại chữ ký sau khi cầm khoá thì B copy LẶP một lượt vô ích.
# Ép thứ tự bằng threading.Event (không sleep nên không phập phù): tạo brain fresh (lần gọi
# ĐẦU TIÊN nên chắc chắn cache trống -> chắc chắn "cây đã đổi" với MỌI luồng), giữ luồng T2
# đứng lại đúng tại _mirror_lock (patch tạm hàm này) cho tới khi luồng T1 chạy xong TOÀN BỘ
# (join thật, không đoán), rồi mới thả T2 ra xin khoá. Skill chỉ 1 file (SKILL.md) nên đếm
# copy2 = đếm chính xác số LƯỢT copy: một lượt đúng phải ra đúng 1, hai lượt (bug) ra 2.
...
check(f"2 luồng đua khi cây ĐÃ đổi -> chỉ MỘT lượt copy thật (đếm được {len(_race_copies)})",
      len(_race_copies) == 1)

# ---- dọn ----
for _d in (_ROOT, _R2, _R3):
