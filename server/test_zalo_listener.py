...
# Từ khi có chế độ chatbot, engine CÓ tham gia. Bất biến đổi từ "không bao giờ gọi engine"
# thành "chỉ gọi trong hộp cát", nên phải chốt lại cho đúng thứ đang bảo vệ.
check("hộp cát: engine chỉ được gọi ở đúng một chỗ là bot_reply",
      src.count("claude_engine(") == 1 and "def bot_reply" in src)

# BẪY THẬT: claude_sdk_engine.py:300 viết `if self.allowed_tools:` - list RỖNG là falsy
# nên allowed_tools=[] rơi xuống else và đặt permission_mode="bypassPermissions" kèm nạp
# settings máy + MCP sẵn có. Tức cách viết trực giác nhất để tạo hộp cát lại MỞ TOANG
# mọi quyền, đúng lúc nội dung người lạ đi vào engine.
check("hộp cát: whitelist tool phải KHÁC RỖNG (list rỗng là falsy = mở toang quyền)",
      bool(zl.NO_TOOL) and "allowed_tools=[NO_TOOL]" in src)
import claude_sdk_engine as _cse  # noqa: E402
_probe = _cse.ClaudeSDK(allowed_tools=[zl.NO_TOOL])
check("hộp cát: whitelist của bot là truthy nên gate quyền THỰC SỰ bật",
      bool(_probe.allowed_tools))
check("hộp cát: gate từ chối tool thật (Bash không nằm trong whitelist)",
      not any(t == "Bash" for t in (_probe.allowed_tools or [])))
check("hộp cát: MCP bị khoá strict nên bot không thấy connector nào",
      "mcp_strict = True" in src)
check("hộp cát: bot bị giới hạn thời gian, không treo cả listener",
      "max_wall_s = 60" in src)

# Model KHÔNG có tool nào nên không tự chọn được người nhận: nó chỉ sinh CHỮ, code Javis
# mới gửi, và luôn gửi về đúng cuộc chat nguồn (tid lấy từ ev, không lấy từ đầu ra model).
check("hộp cát: người nhận do CODE quyết, không lấy từ đầu ra của model",
      'send_zalo(deps, cfg.get("conn_id", ""), tid,' in src)
check("hộp cát: có lối thoát [CHUYEN CHU] để đẩy về chủ thay vì trả lời bừa",
      zl.ESCALATE_MARK in src and "text.startswith(ESCALATE_MARK)" in src)
...
check("connector: bộ dọn quét CẢ tiến trình 'mcp' chứ không chỉ 'listen'",
      '"listen" in line or "mcp" in line' in src3)
